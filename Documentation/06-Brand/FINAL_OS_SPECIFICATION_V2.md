...
### Official Engineering Blueprint · Version 2.0 (Implementation-Ready)

**Document status:** Canonical  
**Supersedes:** `os-architecture-blueprint.md`, `ux-design.md`, `arbor-os-security-architecture.md`, `arbor-os-developer-ecosystem.md`, `arbor-os-performance-engineering.md`, `FINAL_OS_SPECIFICATION.md` (v1.0)  
**Maintained by:** Core OS Team  
**Last consolidated:** 2026-07 (v2.0 revision following independent QA review)
...
> **v2.0 revision note.** Version 1.0 was put through an independent technical QA pass (`FINAL_OS_REVIEW.md`). That review found two hard contradictions that would have produced non-bootable or CI-inconsistent results if implemented as written, several internally inconsistent numbers, and a handful of scope gaps common to Linux desktop specs. All findings assessed as valid have been folded directly into the relevant sections below; none of them change the product vision, the ten Design Conflict rulings in §2, or the roadmap phase structure. A consolidated changelog is provided in §0 immediately below for reviewers who only need the delta.

---

## 0. CHANGELOG — V1.0 → V2.0

The following corrections were applied following QA review. Each references the section it lives in; no other content in this document changed.

| # | Area | v1.0 issue | v2.0 fix | Section |
|---|---|---|---|---|
| 1 | ESP filesystem | Conflict 6 text said "ext4 for the ESP," contradicting FAT32/vfat specified elsewhere — ext4 is not a valid UEFI ESP filesystem | Corrected to FAT32, with rationale, so all references agree | §2 Conflict 6, §3.5, §10.1 |
| 2 | Boot menu vs. boot time | 5-second boot menu timeout appeared to be in the critical path of every boot, contradicting the <8s / 6.8s measured target | Clarified menu is hidden by default and only appears (with timeout) on held key or prior boot failure | §3.6 |
| 3 | Scheduler default | EEVDF and `scx_rustland` were both stated as "the default" scheduler | Clarified EEVDF is the kernel fallback; `scx_rustland` is the loaded session default | §6.3 |
| 4 | RAM CI-gate gap | Target (≤1.2/1.6 GB) and CI block threshold (≥1.4/1.8 GB) differed by 200 MB with no stated reason | Added explicit tolerance-buffer rationale | §6.2, §10.2 |
| 5 | Distrobox `unconfined_u` scope | Exception to the SELinux hard rule wasn't bounded | Added explicit list of controls that remain active around unconfined containers | §8.9 |
| 6 | Minimum hardware spec | No minimum RAM/storage/CPU floor existed despite RAM-sensitive targets | Added explicit minimum/recommended hardware tiers | §3.10 |
| 7 | Camera HCL coverage | No HCL entry for MIPI-CSI/IPU camera stacks, despite Privacy Indicator depending on camera detection | Added Camera row to HCL table | §3.10 |
| 8 | Input method framework | 15-language localization target had no CJK/Indic input method story | Added IBus as the input-method framework, wired into onboarding | §3.4, §4.6 |
| 9 | Snapshot disk-space safeguard | Retention was time-based only, risking silent disk-fill on small SSDs | Added free-space-based pruning safeguard and Settings warning | §3.9, §4.5 |
| 10 | Upstream-tracking ownership | Three fast-moving upstream dependencies were "tracked closely" with no cadence or owner | Added explicit review cadence and ownership | §3.12 (new) |
| 11 | Onboarding "Print" button | Recovery-passphrase Print option assumed a configured printer that doesn't exist yet at Step 4 | Replaced with save-to-USB / QR options; Print offered only if a printer is already available | §4.6 |
| 12 | Enterprise identity/fleet story | "Enterprise trust" goal (Phase 4) had no directory/MDM answer | Added explicit scoping note (deferred to post-1.0, same image mechanism) | §8.5 |
| 13 | Printing/scanning | CUPS/SANE existed only in the AppArmor exception list, with no user-facing Settings/HCL entry | Added Printers & Scanners Settings panel and HCL row | §4.5, §3.10 |
...
**Final decision:** **ZFS is removed from the desktop edition specification.** ZFS support will be specified in a future "Arbor Server Edition" document (out of scope for this specification). Desktop edition filesystems: Btrfs for root and home, **FAT32 for the ESP** (required — UEFI firmware can only read FAT12/16/32 from the EFI System Partition; this is a firmware constraint, not a preference, and must never be changed to ext4 or any other filesystem), no ZFS. Users who require ZFS can use Distrobox containers targeting a ZFS-capable image for storage-only workflows.
...
- **`IBus`** — the input method framework, pre-installed and enabled by default (new in v2.0). Required for the Phase 5 (§9) "top 15 languages" localization commitment: CJK (Chinese/Japanese/Korean), Indic scripts, and other non-Latin input are not usable without a working input method, so this is treated as base-system infrastructure rather than an optional package. Auto-configured with the appropriate engine (e.g., Pinyin, Mozc, Hangul) based on the language selected in onboarding Step 1 (§4.6); switching engines is available in Settings → Personalization → Input Sources.
...
**Menu visibility (clarified for CI consistency):** The boot menu is **hidden by default** — a normal boot proceeds straight to the current deployment with no menu drawn and no timeout consumed. The menu (and its 5-second timeout) is only shown when: (a) a key is held during POST, or (b) the previous boot failed and automatic fallback is offering a choice. This is standard `systemd-boot` behavior and is what the <8 s cold boot target and the 6.8 s measured result in §6.2 assume — the menu timeout is never in the critical path of a normal boot.
...
**Disk-space safeguard (new in v2.0):** Time-based retention alone can silently fill a disk on smaller SSDs. In addition to the configurable time-based retention above, Arbor enforces a **default free-space floor of 15%**: if free space on the Btrfs volume drops below this floor, the oldest non-pinned snapshots (update/layering snapshots and `/home` rolling snapshots alike) are pruned automatically, oldest-first, until free space is restored — pre-update snapshots required for an in-progress rollback are never pruned. Users can pin specific snapshots to exempt them from automatic pruning. A low-space warning surfaces in Settings → Updates & Rollback (§4.5) before automatic pruning is needed, giving the user a chance to intervene manually first.
...
**Minimum and recommended hardware tiers (new in v2.0):** All published benchmarks (§6.2) are measured on the 32 GB reference-tier machine; the floor below is what HCL certification and QA are actually run against, so users and OEM partners have a stated baseline.

| Tier | RAM | Storage | CPU | Notes |
|---|---|---|---|---|
| **Minimum (Standard Mode)** | 8 GB | 64 GB NVMe/SSD (SATA SSD supported, HDD not recommended) | x86_64, 4 cores, last ~10 years | Boots and runs Standard Mode; Developer Mode not certified at this tier |
| **Recommended** | 16 GB | 256 GB NVMe | x86_64, 6+ cores | Developer Mode, gaming, and creative-workflow targets are measured/certified here |
| **Reference (benchmark hardware)** | 32 GB | NVMe (WD SN850X) | AMD Ryzen 7 7840U | Used for all published §6.2 benchmark figures — not a minimum spec |
...
| **Camera (UVC)** | Verified | Standard USB Video Class webcams; backs the Privacy Indicator (§4.2, §5.4) |
| **Camera (MIPI-CSI/IPU, e.g., Intel IPU6)** | Verified per model in HCL | `libcamera` + platform ISP driver; tiered "Limited" where upstream driver maturity is incomplete, matching other DKMS-adjacent hardware |
...
| **Printers/Scanners** | Verified for driverless IPP/eSCL; per-model otherwise | CUPS (driverless-first) + SANE; see §4.5 for the Settings panel |
...
### 3.12 Upstream Tracking Ownership (new in v2.0)

Three dependencies are explicitly the fastest-moving and highest compatibility-break risk in the stack. Each previously said only "tracked closely with upstream" without a cadence or owner; both are now defined:

| Dependency | Why it's high-risk | Review cadence | Compatibility-break response |
|---|---|---|---|
| sched-ext (SCX) BPF schedulers (§6.3) | Relatively young kernel subsystem; policy behavior can shift across kernel point releases | Re-validated on every `arbor-kernel` point-release bump | Automatic fallback to EEVDF (§6.3) is always live; a failing SCX policy is disabled for that kernel build until re-validated, never blocks a kernel update |
| GNOME Shell Extension API v1 wrapping (§2 Conflict 7, §8.8) | Wraps GNOME Shell internals; upstream Shell API churn is the classic source of Linux desktop shell breakage | Re-validated on every GNOME Shell minor version bump, before that version is promoted past Canary (§3.9) | A break blocks promotion to Beta/Stable until the wrapper is patched; Extension API v1's stable contract (§8.8) is the isolation layer that should absorb most churn |
| NVIDIA Wayland explicit-sync (§3.7, §4.2 risk table) | Depends on coordinated NVIDIA driver + Mutter releases | Re-validated on every NVIDIA driver release and every Mutter minor version bump | Dedicated NVIDIA QA lane (§3.7) gates promotion; fallback to open kernel modules/nouveau if a regression can't be resolved before a release deadline |

Each row's compatibility-break response is a release-blocking gate for the Update Delivery Tier Model (§3.9), not merely a monitoring task — this closes the "ownerless technical debt" gap identified in QA review.
...
| **System** | Printers & Scanners | Driverless IPP/AirPrint-style auto-discovery (CUPS) as the default path, per-model fallback drivers where needed; scanning via SANE with a simple scan-to-PDF/image flow (new in v2.0) |
| **Personalization** | Input Sources | Language/input-engine selection (IBus), matching languages chosen in onboarding Step 1 (new in v2.0) |
...
| **Security & Privacy** | Updates & Rollback | Deployment status, one-click rollback, snapshot timeline, free-space indicator with low-space warning before automatic snapshot pruning triggers (new in v2.0, see §3.9) |
...
Step 1 — Language & Region
    ...
    If the selected language requires an input method (CJK, Indic, etc.), the matching
    IBus engine is installed and enabled automatically — no separate setup step (new in v2.0).
...
│  Recovery Passphrase: [ ██████████████ ]                │
│  [Copy]  [Save to USB Drive]  [Show as QR Code]         │
...
No onboarding step requires a cloud account. Recovery passphrase is shown once and the installer refuses to silently skip it. Step 4's passphrase-preservation options (Copy / Save to USB Drive / Show as QR Code, revised in v2.0) are all guaranteed to work with zero prior setup; a "Print" option is additionally offered only when a printer is already configured and discoverable, since no printer setup has occurred this early in first boot.
...
**CI gate tolerance (clarified):** The idle RAM *targets* in the table above (≤ 1.2 GB Standard / ≤ 1.6 GB Developer) are what is marketed, reviewed each release, and used for the reference benchmark. The CI *block* threshold carries a deliberate **+200 MB tolerance** (see §10.2: ≥ 1.4 GB / ≥ 1.8 GB) so that normal benchmark-to-benchmark noise in `smem`/QEMU measurement doesn't flake an otherwise-healthy release. A build that lands between the target and the block threshold ships but triggers a review comment on the PR; a build at or beyond the block threshold fails CI outright.
...
**Scheduler:** EEVDF (Earliest Eligible Virtual Deadline First, Linux 6.6+) is the kernel's built-in scheduler and the **resting-state fallback** whenever no sched-ext policy is loaded (e.g., if SCX BPF loading fails or is disabled). **sched-ext (SCX)** provides per-workload extensible scheduling on top of that fallback, and `scx_rustland` is loaded as the **desktop-session default** at session start — meaning a normal running session is scheduled by `scx_rustland`, not bare EEVDF, unless SCX is unavailable:

| Policy | Use Case | Activation |
|---|---|---|
| `scx_rustland` | General desktop, lowest latency | Loaded at session start (session default) |
| `scx_lavd` | Gaming, latency-sensitive workloads | Auto-applied by GameMode, replacing `scx_rustland` for the session |
| `scx_bpfland` | Power-saving, background tasks | Auto-applied in Battery Saver profile, replacing `scx_rustland` for the session |

If no SCX policy can be loaded (unsupported kernel build, BPF disabled, etc.), the system transparently continues on EEVDF with no user-visible failure — this is the fallback path, not a supported steady state.
...
**Enterprise identity and fleet management scope (clarified in v2.0):** This custom-image mechanism answers *how a fixed configuration gets built and deployed*; it does not by itself answer *how an IT department manages identity, compliance, or lost-device response across a fleet*. Directory/SSO integration (Kerberos/AD/LDAP), MDM-style fleet enrollment, and remote wipe are **explicitly out of scope for 1.0**. They are called out here rather than left implicit, and are expected to be revisited post-GA using the same custom-image/`bootc switch` mechanism as the delivery vehicle (e.g., a directory-integration layer shipped as an additional `rpm-ostree` layer or a dedicated "Arbor Enterprise" image variant). Phase 4's "professional/enterprise trust" goal (§9) refers to security posture and audit readiness, not fleet identity management, which remains a distinct, later workstream.
...
**Scope of the `unconfined_u` exception:** Running Distrobox containers in `unconfined_u` SELinux context removes SELinux enforcement *for processes inside that container only*. It does not remove the other layers around it, which remain fully active and are what actually bound the blast radius in practice:
- **Rootless Podman user-namespace remapping** — container root maps to an unprivileged host UID; a compromised process cannot gain real root on the host even though it is "unconfined" from SELinux's perspective.
- **Host image immutability** — the container cannot write to the read-only OSTree-managed base system regardless of its own SELinux context.
- **No root daemon** — there is no privileged, always-on daemon for a container escape to pivot into (unlike a classic dockerd model).
- **`fapolicyd`** continues to allowlist execution on the host outside any container's mount namespace.

This is the same threat model container runtimes on any SELinux-enforcing distribution operate under; `unconfined_u` is a documented, scoped trade-off for developer ergonomics, not a general weakening of §7.5's SELinux hard rule.
...
*Idle RAM block thresholds intentionally sit 200 MB above the §6.2 targets as a measurement-noise tolerance buffer, not a separate, looser target — see §6.2 for the rationale.*

| Metric | Gate Type | Threshold |
|---|---|---|
| Cold boot time | Block release | ≥ 8.0 s |
| Idle RAM (Standard Mode) | Block release | ≥ 1.4 GB PSS (target: ≤ 1.2 GB, see §6.2) |
| Idle RAM (Developer Mode) | Block release | ≥ 1.8 GB PSS (target: ≤ 1.6 GB, see §6.2) |
...
*This document (v2.0) is the canonical engineering specification for Arbor OS and is implementation-ready: the contradictions and gaps identified in the v1.0 QA review (`FINAL_OS_REVIEW.md`) have been resolved directly in the sections above, with a full changelog in §0. Amendments require review by the Core OS Team and must be reflected here before taking effect in implementation. Superseded documents, including v1.0, are archived but no longer authoritative.*
