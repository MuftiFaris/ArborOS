...
#!/bin/bash
set -e
dnf install -y genisoimage syslinux squashfs-tools dracut-live e2fsprogs pciutils usbutils lshw dmidecode
cd /tmp
mkdir -p build/work/rootfs build/iso/boot build/iso/isolinux build/iso/LiveOS
echo [2/9] Bootstrapping Fedora...
dnf --installroot=/tmp/build/work/rootfs --releasever=39 --setopt=install_weak_deps=False --setopt=cachedir=/tmp/dnf-cache --nodocs -y groupinstall 'Minimal Install'
echo [3/9] Installing packages...
dnf --installroot=/tmp/build/work/rootfs --releasever=39 --setopt=cachedir=/tmp/dnf-cache -y install kernel systemd NetworkManager openssh-server sudo nano bash-completion dracut-live dracut-network linux-firmware mesa-dri-drivers pipewire wireplumber alsa-utils lxqt-panel lxqt-session lxqt-config openbox pcmanfm-qt qterminal featherpad lightdm network-manager-applet breeze-icon-theme
...
chroot /tmp/build/work/rootfs dracut --force --no-hostonly --add 'dmsquash-live livenet' --omit plymouth /boot/initramfs-live.img `$KVER
...
# Write with Unix LF line endings
$utf8NoBom = New-Object System.Text.UTF8Encoding $false
[System.IO.File]::WriteAllLines("$PSScriptRoot\build-script.sh", $buildScript, $utf8NoBom)

docker run --rm -it --privileged -v "${PWD}:/workspace" fedora:39 bash /workspace/Code/build/build-script.sh

Remove-Item "$PSScriptRoot\build-script.sh" -ErrorAction SilentlyContinue
